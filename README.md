...
The commands below describe the original training environment. The revised
implementation's CPU regression suite was verified on Windows with Python 3.12.14
and PyTorch 2.14.0; the old PyTorch 1.7 environment has not been revalidated.
For the CPU checks, an isolated modern environment needs `torch`, `numpy`,
`scipy`, `matplotlib`, `pandas`, `PyYAML`, `tqdm`, `tensorboardX`, and `pytest`.
