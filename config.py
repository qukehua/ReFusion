...
def update_config(cfg, args_dict):
    ...
    if cfg.t_his < 1 or cfg.t_pred < 1:
        raise ValueError('t_his and t_pred must be positive.')
    if cfg.n_pre is None:
        cfg.n_pre = cfg.t_pred
    if not 1 <= cfg.n_pre <= cfg.t_pred:
        raise ValueError('n_pre must be in [1, t_pred]; it counts future DCT coefficients.')
    if cfg.Complete:
    ...
    for suffix, length in (('obs', cfg.t_his), ('pred', cfg.t_pred)):
        dct, idct = util.get_dct_matrix(length)
        setattr(cfg, 'dct_m_' + suffix, dct.to(device=cfg.device, dtype=dtype))
        setattr(cfg, 'idct_m_' + suffix, idct.to(device=cfg.device, dtype=dtype))

    exp_base_name = getattr(cfg, 'exp_name', None) or args_dict['cfg']
    if args_dict['mode'] in ('train', 'pred', 'eval'):
...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        self.adam_betas = tuple(cfg.get('adam_betas', [0.9, 0.999]))
        self.adam_eps = cfg.get('adam_eps', 1e-8)
        ...
        # Full future spectrum by default, matching P in the paper.
        self.n_pre = cfg.get('n_pre', self.t_pred)
        self.multimodal_path = cfg.get('multimodal_path', None)
        self.data_candi_path = cfg.get('data_candi_path', None)

        self.padding = cfg['padding']
        self.Complete = cfg.get('Complete', False)
        ...
        self.use_partner_condition = cfg.get('use_partner_condition', True)
        ...
        if self.dataset == 'harper3d':
            ...
            self.output_total_joints = 24 if self.predict_human_only else self.total_joint_num
            self.joint_num = self.output_total_joints - 1
            ...
            if (not isinstance(self.cmu_person_joint_num, int)
                    or self.cmu_person_joint_num < 2
                    or self.total_joint_num != 2 * self.cmu_person_joint_num):
                raise ValueError(
                    'CMU requires two equally sized skeletons: cmu_total_joint_num must equal '
                    '2 * cmu_person_joint_num, with at least two joints per person.'
                )
            self.output_total_joints = self.cmu_person_joint_num if self.predict_human_only else self.total_joint_num
            self.joint_num = self.output_total_joints - 1
            self.cond_joint_num = self.total_joint_num - 1 if self.use_partner_condition else self.joint_num
        else:
            raise ValueError(
