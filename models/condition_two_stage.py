...
class GraphConvBlock(nn.Module):
    """Eq. (2) on [B, T_obs, J_h, C_in], with two learned adjacencies.

    Applying the frequency and spatial matrices separately is equivalent to
    their Kronecker product without materializing a (T_obs * J_h)^2 matrix.
    """

    def __init__(self, num_nodes, hidden_size, num_frames, dropout=0.0, input_size=None):
        super().__init__()
        self.num_nodes = num_nodes
        self.num_frames = num_frames
        self.input_size = hidden_size if input_size is None else input_size
        self.spatial_adj = nn.Parameter(torch.eye(num_nodes))
        self.frequency_adj = nn.Parameter(torch.eye(num_frames))
        self.norm = nn.LayerNorm(self.input_size)
        self.channel_proj = nn.Linear(self.input_size, hidden_size, bias=False)
        self.activation = nn.ReLU()
        self.drop = nn.Dropout(dropout)

    def forward(self, x):
        expected = (self.num_frames, self.num_nodes, self.input_size)
        if x.ndim != 4 or tuple(x.shape[1:]) != expected:
            raise ValueError(
                f"Graph input must be [B, {self.num_frames}, {self.num_nodes}, {self.input_size}], "
                f"got {tuple(x.shape)}."
            )
        x = self.channel_proj(self.norm(x))
        x = torch.einsum('ij,btjc->btic', self.spatial_adj, x)
        x = torch.einsum('st,btjc->bsjc', self.frequency_adj, x)
...
class MotionTransformerTwoStage(nn.Module):
    """Future-only DCT denoiser with independently encoded observed conditions.

    x: [B, K, 3 * J_h], with K <= num_frames future DCT coefficients.
    mod: [B, cond_frames, 3 * (J_h + J_r)] observed DCT coefficients.
    Relation features [B, cond_frames, J_h, d] are mapped to future tokens
    [B, num_frames, J_h, d] by learned projections along the frequency axis.
    The first K projected tokens are used when future DCT is truncated.
    """
    ...
    def __init__(self,
        ...
        self.num_frames = num_frames
        self.cond_frames = num_frames if cond_frames is None else cond_frames
        ...
        if not isinstance(self.cond_frames, int) or self.cond_frames < 1:
            raise ValueError(f"cond_frames must be a positive integer, got {self.cond_frames}.")
        ...
        if self.human_cond_joint_num != self.input_joint_num:
            raise ValueError(
                "The target human and observed human must have the same joints for token addition: "
                f"got {self.input_joint_num} and {self.human_cond_joint_num}."
            )
        ...
        if stage1_num_layers is None:
            ...
            self.stage1_num_layers = stage1_num_layers
        if not 0 <= self.stage1_num_layers <= num_layers:
            raise ValueError(
                f"stage1_num_layers must be in [0, {num_layers}], got {self.stage1_num_layers}."
            )
        self.dit_attn_mode = dit_attn_mode
        ...
        self.human_gcn = nn.ModuleList([
            GraphConvBlock(self.human_cond_joint_num, latent_dim, self.cond_frames,
                           dropout=dropout, input_size=3),
            GraphConvBlock(self.human_cond_joint_num, latent_dim, self.cond_frames,
                           dropout=dropout),
        ])
        ...
        # Eq. (11)/(18) require observation and future frequency axes to agree.
        # Channel projections alone cannot align T_obs with the prediction horizon.
        self.human_frequency_proj = nn.Linear(self.cond_frames, num_frames)
        self.interaction_frequency_proj = (
            nn.Linear(self.cond_frames, num_frames) if self.robot_cond_joint_num > 0 else None
        )
        ...
        self.cond_time_pos_embed = nn.Parameter(torch.zeros(1, self.cond_frames, latent_dim), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        self.cond_time_pos_embed.data.copy_(get_1d_sincos_pos_embed(self.cond_frames, self.latent_dim))
        ...
        for proj in (self.human_frequency_proj, self.interaction_frequency_proj):
            if proj is not None:
    ...
    @staticmethod
    def align_condition_frequency(tokens, projection, num_coefficients):
        """[B,T,J,d] -> Linear(T,P) -> [B,K,J,d], K <= P."""
        return projection(tokens.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)[:, :num_coefficients]

    def encode_human_condition(self, mod, num_coefficients=None):
        bsz, seq_len, _ = mod.shape
        num_coefficients = self.num_frames if num_coefficients is None else num_coefficients
        human_dim = self.human_cond_joint_num * 3
        human_motion = mod[:, :, :human_dim].reshape(bsz, seq_len, self.human_cond_joint_num, 3)
        # The intra branch follows Eq. (2), starting with C_0 = 3 coordinates.
        intra_tokens = human_motion
        for gcn in self.human_gcn:
            intra_tokens = gcn(intra_tokens)
        time_pe = self.cond_time_pos_embed[:, :seq_len].unsqueeze(2)
        intra_tokens = intra_tokens + time_pe + self.human_joint_pos_embed
        stage1_tokens = self.align_condition_frequency(
            self.human_token_proj(intra_tokens), self.human_frequency_proj, num_coefficients
        )
        human_global = self.human_global_proj(self.human_global_pool(
            stage1_tokens.reshape(bsz, num_coefficients * self.human_cond_joint_num, self.latent_dim)
        ))
        # Eq. (3): project DCT xyz to d before adding [1,T,1,d] and [1,1,J,d].
        # Interaction queries are observed coordinates, independent of the GCN.
        human_tokens = self.human_joint_embed(human_motion) + time_pe + self.human_joint_pos_embed
        return human_tokens, stage1_tokens, human_global

    def encode_interaction_condition(self, mod, human_tokens, num_coefficients=None):
        bsz, seq_len, _ = mod.shape
        num_coefficients = self.num_frames if num_coefficients is None else num_coefficients
        if self.robot_cond_joint_num == 0:
            # A learned projection of zero tokens can have a nonzero bias; omit
            # that path entirely so human-only input has no fabricated interaction.
            zero_tokens = human_tokens.new_zeros(
                bsz, num_coefficients, self.human_cond_joint_num, self.latent_dim
            )
            zero_global = human_tokens.new_zeros(bsz, self.latent_dim)
        ...
        stage2_tokens = self.align_condition_frequency(
            self.interaction_token_proj(temporal_interaction),
            self.interaction_frequency_proj, num_coefficients,
        )
    ...
    def forward(self, x, timesteps, mod=None):
        if x.ndim != 3:
            raise ValueError(f"Input x must be [B, K, D_h], got shape {tuple(x.shape)}.")
        bsz, seq_len, feat_dim = x.shape
        if not 1 <= seq_len <= self.num_frames:
            raise ValueError(f"Future DCT length must be in [1, {self.num_frames}], got {seq_len}.")
        ...
        if mod is not None:
            ...
            if mod.shape[1] != self.cond_frames:
                raise ValueError(
                    f"Observed DCT length mismatch: mod T={mod.shape[1]} vs cond_frames={self.cond_frames}."
                )
            if mod.shape[2] != self.cond_feats:
                raise ValueError(
                    f"Condition feature mismatch: mod D={mod.shape[2]} vs expected cond_feats={self.cond_feats}."
                )
            human_tokens, stage1_tokens, human_global = self.encode_human_condition(mod, seq_len)
            c_stage1 = c_base + human_global
            c_stage2 = c_stage1
            if self.stage1_num_layers < self.num_layers:
                stage2_tokens, interaction_global = self.encode_interaction_condition(mod, human_tokens, seq_len)
                c_stage2 = c_base + human_global + interaction_global  # Eq. (19).
            # Eq. (11) still initializes tokens when L_I = 0; the transition
            # to stage II then occurs before block 0. L_II = 0 has no transition.
            x = x + stage1_tokens
        ...
        final_condition = c_stage1 if self.stage1_num_layers == self.num_layers else c_stage2
        x = self.final_layer(x, final_condition)
